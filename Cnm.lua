...
function MemoryManager:CleanupExcessObjects()
    for className, pool in pairs(self.objectPools) do
        while #pool > self.maxPoolSize do
...
spawn(function()
    while task.wait(MemoryManager.cleanupInterval) do
        MemoryManager:CleanupExcessObjects()
    end
end)


function MemoryManager:GetFromPool(className)
    if not self.objectPools[className] then
        self.objectPools[className] = {}
    end
    
    if #self.objectPools[className] > 0 then
        local obj = table.remove(self.objectPools[className])
...
function MemoryManager:ReturnToPool(obj)
    if self.activeObjects[obj] then
        ...
        table.insert(self.objectPools[className], obj)
...
function PerformanceMonitor:Update()
    ...
    while #self.frameTimes > 0 and currentTime - self.frameTimes[1] > 1 do
        table.remove(self.frameTimes, 1)
    end
    
    if currentTime - self.lastCheck >= self.checkInterval then
        self.lastCheck = currentTime
        local fps = #self.frameTimes
        if fps < 30 then
            self:AdjustQuality(fps)
        end
    end
end

function PerformanceMonitor:AdjustQuality(fps)
    
    local qualityLevel = math.floor(fps / 15) 
    _G.UI_QUALITY = qualityLevel
    
    if qualityLevel <= 1 then
        _G.DISABLE_PARTICLES = true
        _G.DISABLE_ANIMATIONS = true
        _G.REDUCE_GRADIENTS = true
    elseif qualityLevel <= 2 then
        
        _G.DISABLE_PARTICLES = false
        _G.DISABLE_ANIMATIONS = false
        _G.REDUCE_GRADIENTS = false
    else
        _G.DISABLE_PARTICLES = false
        _G.DISABLE_ANIMATIONS = false
        _G.REDUCE_GRADIENTS = false
...
local minFrameTime = 1/MAX_FPS

local function safeRender(callback)
    local currentTime = os.clock()
    if currentTime - lastRenderTime >= minFrameTime then
...
local hologramProjector = Instance.new("Frame")
hologramProjector.Name = "HologramProjector"
hologramProjector.Parent = new[2]
hologramProjector.BackgroundColor3 = Color3.fromRGB(0, 255, 255)
hologramProjector.BackgroundTransparency = 0.9
hologramProjector.Size = UDim2.new(1, 0, 1, 0)
hologramProjector.ZIndex = 999

local scanLine = Instance.new("Frame")
scanLine.Parent = hologramProjector
...
local soundVisualizer = Instance.new("Frame")
soundVisualizer.Name = "SoundVisualizer"
soundVisualizer.Parent = MainXE
soundVisualizer.BackgroundTransparency = 1
soundVisualizer.Size = UDim2.new(1, -20, 0, 60)
soundVisualizer.Position = UDim2.new(0, 10, 1, -70)
soundVisualizer.ZIndex = 10

local bars = 30
for i = 1, bars do
    local bar = Instance.new("Frame")
    bar.Name = "Bar_"..i
    bar.Parent = soundVisualizer
    bar.BackgroundColor3 = Color3.fromHSV(i/bars, 1, 1)
    bar.Size = UDim2.new(1/bars, -2, 0, 0)
    bar.Position = UDim2.new((i-1)/bars, 0, 1, 0)
    bar.AnchorPoint = Vector2.new(0, 1)
    
    spawn(function()
        while wait(math.random()*0.1) do
            local targetHeight = math.random(10,60)
            game:GetService("TweenService"):Create(
                bar,
                TweenInfo.new(0.2, Enum.EasingStyle.Quad),
                {Size = UDim2.new(1/bars, -2, 0, targetHeight)}
            ):Play()
        end
    end)
end
...
tween:Play()
    ...
    local shadow1 = ScriptTitle:Clone()
    shadow1.Parent = ScriptTitle.Parent
    shadow1.TextColor3 = Color3.fromRGB(0, 0, 0)
    shadow1.Position = UDim2.new(0, 2, 0.00953488424, 2)
    shadow1.ZIndex = 9
    
    local shadow2 = ScriptTitle:Clone()
    shadow2.Parent = ScriptTitle.Parent
    shadow2.TextColor3 = Color3.fromRGB(50, 50, 50)
    shadow2.Position = UDim2.new(0, 1, 0.00953488424, 1)
    shadow2.ZIndex = 9
    ...
    spawn(function()
        while true do
            game:GetService("TweenService"):Create(
                ...
                {Position = UDim2.new(0, 0, 0.00953488424, -3)}
            ):Play()
            wait(2)
            game:GetService("TweenService"):Create(
                ScriptTitle,
                TweenInfo.new(2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut),
                {Position = UDim2.new(0, 0, 0.00953488424, 0)}
            ):Play()
            wait(2)
        end
    end)
    ...
    local tween = TweenService:Create(OpenGradient, tweenInfo, {Rotation = 360, Offset = Vector2.new(1, 0)})
    tween:Play()
    ...
    function window.Tab(window, name, icon)
        ...
        local function updateSearch()
            local searchText = string.lower(SectionSearchBar.Text)
            ClearButton.Visible = searchText ~= ""
        ...
        function tab.section(tab, name, TabVal)
            ...
            function section.Button(section, text, callback)
                ...
                Btn.BackgroundColor3 = Color3.fromRGB(100, 100, 255) -- 默认颜色
                ...
                Btn.MouseButton1Click:Connect(function()
                    local fracture = Instance.new("Frame")
                    fracture.Parent = Btn
                    fracture.Size = UDim2.new(1, 0, 1, 0)
                    fracture.BackgroundTransparency = 1
            
                    for i = 1, 20 do
                        local fragment = Instance.new("Frame")
                        fragment.Size = UDim2.new(0.2, 0, 0.2, 0)
                        fragment.Position = UDim2.new(math.random(), 0, math.random(), 0)
                        fragment.BackgroundColor3 = Color3.fromHSV(i / 20, 1, 1)
                        fragment.Parent = fracture
            
                        game:GetService("TweenService"):Create(
                            fragment,
                            TweenInfo.new(0.5),
                            {
                                Position = UDim2.new(math.random() * 2 - 0.5, 0, math.random() * 2 - 0.5, 0),
                    ...
                    delay(0.5, function() fracture:Destroy() end)
                end)
                ...
                Btn.MouseEnter:Connect(function()
                    ...
                    hologram.Parent = Btn
                    hologram.BackgroundColor3 = Color3.fromRGB(0, 255, 255)
                    hologram.BackgroundTransparency = 0.7
                    hologram.Size = UDim2.new(1, 0, 1, 0)
                    hologram.ZIndex = -1
            
                    local emitter = Instance.new("ParticleEmitter")
                    emitter.Parent = hologram
                    emitter.LightEmission = 1
                    emitter.Size = NumberSequence.new(0.5)
                    emitter.Texture = "rbxassetid://242503432"
